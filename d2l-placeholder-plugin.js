		...
		addPlugin: function() {
			tinymce.PluginManager.add('d2l_placeholder', function(editor) {
				...
				editor.on('init', function() {
					...
					editor.on('blur change setContent keyup', function() { onBlur(placeholderElement); });
					...
					function onBlur(placeholderElement) {
						if (!(document.activeElement && document.activeElement.isEqualNode && document.activeElement.isEqualNode(editor.getElement())) &&
							(editor.getContent() === '' || editor.getContent({format: 'raw'}) === editor.startContent)) {
